    ...
    class Program
    {
        public static void Main(string[] args)
        {
            ...
            Player playerOne = new Player(Helper.DuplicateBoatList(gameConfig.Boats), 
                ...
                new Gameboard(gameConfig.Lines, gameConfig.Columns), playerOneName);

            Player playerTwo = new Player(Helper.DuplicateBoatList(gameConfig.Boats),
                new Gameboard(gameConfig.Lines, gameConfig.Columns),
                new Gameboard(gameConfig.Lines, gameConfig.Columns), playerTwoName);

           
            // 3- Place boats on the gameboards
            Extras.TurnTransition($"{playerOne.name}'s turn to place boat");
            Console.WriteLine($"{playerOne.name}'s turn to place boat");
            Extras.PlaceBoats(playerOne);

            Extras.TurnTransition($"{playerTwo.name}'s turn to place boat");
            Console.WriteLine($"{playerTwo.name}'s turn to place boat");
